...
public class Mine {
    ...
    public Mine(int x, int y, int mapCol, int mapRow, int mineNum, int tileWidth)
    {
        ...
        textPaint.setTextSize(MainActivity.W/15);
        textPaint.setARGB(60,47,85,85);//数字颜色


        bmpPaint=new Paint();
        bmpPaint.setAntiAlias(true);
        bmpPaint.setARGB(90,26,34,42);//靁颜色
        bmpPaint.setPathEffect(new DiscretePathEffect(3.0F, 15.0F));
        //bmpPaint.setPathEffect(new DashPathEffect(new float[] {20, 10}, ));
        bmpPaint.setStyle(Paint.Style.STROKE);
        bmpPaint.setStrokeWidth(5);
        bmpPaint.setShadowLayer(10,5,5,Color.BLUE);



        tilePaint =new Paint();
        tilePaint.setAntiAlias(true);
        tilePaint.setMaskFilter(new BlurMaskFilter(20, BlurMaskFilter.Blur.SOLID));
        tilePaint.setColor(Color.rgb(182,193,188));//第一层背景颜色

        /*minePaint =new Paint();
        minePaint.setAntiAlias(true);
        minePaint.setColor(0xffff981d);
        bmpPaint.setStrokeCap(Paint.Cap.SQUARE);*/



        rectPaint =new Paint();
        rectPaint.setAntiAlias(true);
        rectPaint.setColor(Color.rgb(127,163,159));//地图线颜色
